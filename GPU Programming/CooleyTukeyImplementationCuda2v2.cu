...
__global__ void sumEvenOddS(ComplexNum* evenOddR, ComplexNum* XkResults, int N, int k);
...
__global__ void FFT_Kernel(ComplexNum* signals_d, ComplexNum* evenOddR, int N, int k)
{
	...
	double theta = (-2 * PI * k) / (N / 2);
	 if(m < (N / 2))
	 {
		...
		//if(k == 0 || k == 1)
		//	printf("K[%d] Xreal[%d] = %lf, Ximag[%d] = %lf\n", k, m, evenOddR[(k * (N/2)) + m].real ,m, evenOddR[(k * (N/2)) + m].imag );
	}
...
__global__ void FFT_KernelSuper(ComplexNum*signals_d, ComplexNum* XkResults, ComplexNum* evenOddR, int N)
{
	...
	sumEvenOddS <<<1, 1024 >>>(evenOddR, XkResults, N, k);
	cudaDeviceSynchronize();
